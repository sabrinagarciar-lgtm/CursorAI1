...
const FEATURES = [
  { to: "/search", title: "Product Search", desc: "Browse, filter, and shop from Exercises 5 & 8.2", color: "from-indigo-500 to-violet-600" },
...
export function HomePage() {
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-indigo-100 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">CursorHub Final Platform</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {FEATURES.map((f) => (
          <Link
            key={f.to}
            to={f.to}
            className="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700 dark:bg-slate-900"
          >
            <div className={`mb-3 h-2 w-12 rounded-full bg-gradient-to-r ${f.color}`} />
            <h2 className="font-semibold text-slate-900 group-hover:text-indigo-700 dark:text-slate-100 dark:group-hover:text-indigo-400">
              {f.title}
            </h2>
