      ...
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-2">
            ...
            <div>
              <p className="text-lg font-bold tracking-tight text-slate-900">CursorHub</p>
              <p className="text-xs text-slate-500">Integrated platform</p>
            </div>
          </Link>
            ...
            {isAuthenticated ? (
              <>
                ...
                <button
                  type="button"
                  onClick={logout}
                  className="rounded-lg px-2.5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100"
                >
                  Sign out
                </button>
              </>
