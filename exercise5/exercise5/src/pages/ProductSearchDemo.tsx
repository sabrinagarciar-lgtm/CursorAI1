...
function readCatalogErrorFlag(): boolean {
  if (typeof window === 'undefined') {
    return false;
  }
  return new URLSearchParams(window.location.search).get('catalogError') === '1';
}

const ProductSearchDemo: React.FC = () => {
  const [catalogError] = useState(() => readCatalogErrorFlag());
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-4 dark:bg-slate-950">
        ...
        >
          ...
          <p
            data-testid="catalog-error-message"
            className="mt-3 text-sm text-slate-600 dark:text-slate-400"
          >
            Something went wrong while fetching products. This is a simulated error for E2E testing
            (open the app without <code className="rounded bg-slate-100 px-1 dark:bg-slate-800">catalogError=1</code>
            ).
          </p>
          <button
            type="button"
